...
def app(car_df):
  ...
  if st.button('Predict'):
    ...
    st.write('The predicted price of the car is ', price)
    st.info(f'Accuracy score of the linear regression model is: {score}')
    st.info(f'Mean Absolute Error for testing dataset: {mae_test}')
    st.info(f'Root Mean Squared Error for testing dataset: {rmse_test}')
    st.info(f'Mean Squared Log Error for testing dataset: {msle_test}')
